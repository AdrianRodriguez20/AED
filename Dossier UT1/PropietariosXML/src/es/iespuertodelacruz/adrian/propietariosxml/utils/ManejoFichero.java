 ...
 */
package es.iespuertodelacruz.adrian.propietariosxml.utils;
...
public class ManejoFichero {
    ...
    public boolean borrarYAgregar(String texto) {
           boolean aniadido = false;
        try (BufferedWriter bw = Files.newBufferedWriter(this.file.toPath(), StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING)) {
            bw.write(texto);
            aniadido = true;
        } catch (IOException ex) {
            aniadido = false;
